...
type Props = {
  ...
  onEdit?: (id: string, title: string) => void; // opcional aqui
  loading?: boolean;
};
...
export default function TaskList({
  ...
  // Mantém ordem estável (mais recentes primeiro), sem mutar a prop
  useEffect(() => {
    if (isInitialLoad) {
      setLocalTasks(tasks);
    ...
    // ordena por createdAt desc; se empatar, usa id pra estabilidade
    const ordered = [...tasks].sort((a, b) => {
      if (b.createdAt !== a.createdAt) return b.createdAt - a.createdAt;
      return a.id.localeCompare(b.id);
    });
    setLocalTasks(ordered);
  }, [tasks, isInitialLoad]); // << inclui deps corretas
        ...
        {[...Array(3)].map((_, i) => (
          ...
          >
            <div className="flex animate-pulse">
              <div className="h-5 w-5 rounded border-gray-300 bg-gray-200 dark:bg-gray-700" />
...
<ul className="mt-6 space-y-3">
  {localTasks.map((task) => (
    <TaskItem
      ...
      onToggle={onToggle}
      onRemove={onRemove}
      // 👇 sempre passa uma função; se não vier, vira no-op
